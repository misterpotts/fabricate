...
class CompendiumEntryImportError extends Error {
  ...
  constructor(
    ...
    const message: string = `Unable to import ${fabricateItem.type} with Fabricate Part ID '${fabricateItem.identity.partId}' from Compendium Entry with ID '${entryId}' specified in Compendium with Pack Key '${compendiumPackKey}' into System '${systemId}'. Caused by: ${cause.message}`;
    super(message);
