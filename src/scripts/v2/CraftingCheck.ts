...
abstract class CraftingCheck<I extends Item, A extends Actor<Actor.Data, I>> {
    ...
    protected abstract getSuccessThreshold(components: Combination<CraftingComponent>): number;

    protected abstract getRollTermData(actor: A): DieData;

    /*
    * This function exists solely for stubbing in test, as Sinon doesn't seem to want to stub the DiceTerm constructor
    * in the gulp build, but will do it in the IDE :shrug: TODO - Look at whether or not Jest is less bad at this than Mocha/Sinon
    * */
    private constructRollTerm(args: DieData): Die {
        return new Die(args);
    }
...
class CraftingCheck5e extends CraftingCheck<Item5e, Actor5e>{
    ...
    protected getSuccessThreshold(components: Combination<CraftingComponent>): number {
        ...
        const dcModifier = components.members.map((component: CraftingComponent) => {
            const essenceContribution: number = essenceCountContributes ? component.essences.size() * this.essenceDCModifier : 0;
    ...
    protected getRollTermData(actor: Actor5e): DieData {
