...
class EssenceContributionCounter implements ContributionCounter {
  ...
  determineDCModifier(components: Combination<CraftingComponent>): number {
    return components
      .explode((component: CraftingComponent) => component.essences)
...
class IngredientContributionCounter implements ContributionCounter {
  ...
  determineDCModifier(components: Combination<CraftingComponent>): number {
    return components.units
      .map((unit: Unit<CraftingComponent>) => this._ingredientModifier * unit.quantity)
      .reduce((left: number, right: number) => left + right, 0);
  }
}
...
abstract class CraftingCheck<A extends Actor> {
  ...
  protected constructor(
    ...
    const contributionCounterFactory: ContributionCounterFactory = new ContributionCounterFactory(
      ingredientCountModifier,
      essenceTypeCountModifier,
    );
