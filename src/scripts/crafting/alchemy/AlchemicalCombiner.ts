...
class AlchemicalCombiner<D> {
  ...
  public constructor(builder: AlchemicalCombiner.Builder<D>) {
    ...
    builder.effects.forEach((effect: AlchemicalEffect<D>) =>
      this._effectsByEssenceCombinationIdentity.set(
        essenceIdentityProvider.getForEssenceCombination(effect.essenceCombination),
        effect,
      ),
    );
  ...
  private validate(components: Combination<CraftingComponent>, essences: Combination<EssenceDefinition>) {
    if (this.maxComponents > 0 && this.maxComponents < components.size()) {
      throw new AlchemyError(
        `The Essence Combiner for this system supports a maximum of ${
          this.maxComponents
  ...
  private determineAlchemicalEffectsForComponents(
    componentCombination: Combination<CraftingComponent>,
  ): [AlchemicalEffect<D>, number][] {
