...
class CraftingCheck5e extends CraftingCheck<Actor5e> {
  ...
  private getModifiers(actor: Actor5e, ability: AbilityType, tool?: Tool): string[] {
    ...
    const hasToolProficiency: string = allToolProficiencies.find(
    ...
    const ownsTool: Item5e = actor.items.find(
      (item: Item5e) =>
        item.type === 'tool' && tool.displayName.localeCompare(item.name, undefined, { sensitivity: 'base' }) === 0,
    );
    if (!ownsTool) {
      return this.convertToModifierExpression(craftingAbility.mod);
    }
