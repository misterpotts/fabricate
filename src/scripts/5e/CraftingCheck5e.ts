...
class CraftingCheck5e extends CraftingCheck<Actor5e> {
    ...
    private getModifiers(actor: Actor5e, ability: AbilityType, tool?: Tool): string[] {
        ...
        const character = actor.data.data;
        //@ts-ignore
        ...
        const ownsTool = <Item>actor.items
            .find((item: Item) => item.type === 'tool' && tool.displayName.localeCompare(<string>item.name, undefined, {sensitivity: 'base'}) === 0);
        if (!ownsTool) {
            return this.convertToModifierExpression(craftingAbility.mod);
        }
        //@ts-ignore
