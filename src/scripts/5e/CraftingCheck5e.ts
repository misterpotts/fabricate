...
class CraftingCheck5e extends CraftingCheck<Actor5e> {
  ...
  private getModifiers(actor: Actor5e, ability: AbilityType, tool?: Tool): string[] {
    ...
    const character = <any>actor.data.data;
    const allToolProficiencies: string[] = character.traits.toolProf.custom
      .split(',')
      .map((customProficiency: string) => customProficiency.trim())
      .concat(character.traits.toolProf.values);
    const hasToolProficiency: string = <string>(
      allToolProficiencies.find(
        (toolProficiency: string) =>
          tool.proficiencyLabel.localeCompare(toolProficiency, undefined, { sensitivity: 'base' }) === 0,
      )
    );
    if (!hasToolProficiency) {
      return this.convertToModifierExpression(craftingAbility.mod);
    }
    const ownsTool: Item5e = <Item5e>(
      actor.items.find(
        (item: Item5e) =>
          item.type === 'tool' &&
          tool.displayName.localeCompare(<string>item.name, undefined, { sensitivity: 'base' }) === 0,
      )
    );
    if (!ownsTool) {
      return this.convertToModifierExpression(craftingAbility.mod);
